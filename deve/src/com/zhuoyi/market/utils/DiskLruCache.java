...
public class DiskLruCache {
	...
	private static final int INITIAL_CAPACITY = 32;//32;
	...
	public static void putImageToDisk(String key,String cacheDir,Bitmap data)
	{
	    try {
           // final String file = createFilePath(cacheDir, key);
	        final String file = cacheDir + File.separator + CACHE_FILENAME_PREFIX + URLEncoder.encode(key.replace("*", ""), "UTF-8");
	...
	private void flushCache() {
		...
		while (count < MAX_REMOVALS && (/*cacheSize > maxCacheItemSize || */cacheByteSize > maxCacheByteSize)) {
			eldestEntry = mLinkedHashMap.entrySet().iterator().hasNext() ? mLinkedHashMap.entrySet().iterator().next() : null;
			if(eldestEntry == null) return;
			//Log.e("shuaiqingDebug", "cacheSize:"+cacheSize+" cacheByteSize:"+cacheByteSize+" flushCache: "+ eldestEntry.getValue());
	 ...
	 * @return
	 */
	private static boolean writeBitmapToFile(Bitmap bitmap, String file) throws IOException, FileNotFoundException {
