...
public class WheelMain {
	...
	private static int START_YEAR = 1900, END_YEAR = 3000, END_MONTH = 12, END_DAY = 30;
...
//		int day = calendar.get(Calendar.DATE);
		...
		else if (list_big.contains(String.valueOf(month + 1))) {
		...
		OnWheelChangedListener wheelListener_year = new OnWheelChangedListener() {
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				...
				if(year_num == END_YEAR && wv_month.getCurrentItem() == END_MONTH)
					wv_day.setAdapter(new NumericWheelAdapter(1, END_DAY));
				else if (list_big.contains(String.valueOf(wv_month.getCurrentItem() + 1))) {
					wv_day.setAdapter(new NumericWheelAdapter(1, 31));
		...
		OnWheelChangedListener wheelListener_month = new OnWheelChangedListener() {
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				...
				// 判断大小月及是否闰年,用来确定"日"的数据
				if((wv_year.getCurrentItem() + START_YEAR) == END_YEAR && month_num == END_MONTH + 1) {
					wv_day.setAdapter(new NumericWheelAdapter(1, END_DAY));
				} else if (list_big.contains(String.valueOf(month_num))) {
					wv_day.setAdapter(new NumericWheelAdapter(1, 31));
