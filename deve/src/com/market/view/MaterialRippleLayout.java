...
public class MaterialRippleLayout extends FrameLayout
{
	...
	private static final long HOVER_DURATION = 2000;
	...
	private AdapterView<Adapter> parentAdapter;
	...
	public MaterialRippleLayout(Context context, AttributeSet attrs, int defStyle)
	{
		...
		rippleBackground = new ColorDrawable(a.getColor(R.styleable.MaterialRippleLayout_mrl_rippleBackground,
				DEFAULT_BACKGROUND));
		...
		rippleRoundedCorners = a.getDimensionPixelSize(R.styleable.MaterialRippleLayout_mrl_rippleRoundedCorners,
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		if (isEventInBounds && event.getAction() == MotionEvent.ACTION_DOWN)
		{
	...
	private void startHover()
	{
		...
		final float _radius = (float) (Math.sqrt(Math.pow(getWidth(), 2) + Math.pow(getHeight(), 2)) * 1.2f);
		hoverAnimator = ObjectAnimator.ofFloat(this, radiusProperty, rippleDiameter, _radius)
	...
	@SuppressWarnings("unchecked")
	private AdapterView<Adapter> findParentAdapterView()
	{
		...
		while (true)
		{
			...
			{
				parentAdapter =  (AdapterView<Adapter>) current;
				return parentAdapter;
			}
			else
			{
	...
	/**
	 * {@link Canvas#clipPath(Path)} is not supported in hardware accelerated layers before API 18. Use
	...
	private class PerformClickEvent implements Runnable
	{
		...
		public void run()
		{
			if (!hasPerformedLongPress)
			{
				...
				if (getParent() instanceof AdapterView)
				{
					...
					isClick = isClickAdapterView((AdapterView<Adapter>) getParent());
				}
				else if (rippleInAdapter)
				{
					// find adapter view
					isClick = isClickAdapterView(findParentAdapterView());
				}
				//the childView will catch this click event when adapterView doesn't consume this click event in listView
				if (!isClick)
				{
		...
		private boolean isClickAdapterView(AdapterView<Adapter> parent)
		{
			...
			{
				return parent.performItemClick(MaterialRippleLayout.this, position, itemId);
			}
			return false;
		}
	}
	...
	private final class PressedEvent implements Runnable
	{
		...
		{
			prepressed = false;
			childView.setLongClickable(false);// prevent the child's long click,let's the ripple layout call
												// it's performLongClick
			childView.onTouchEvent(event);
			childView.setPressed(true);
			if (rippleHover)
			{
	...
	public static class RippleBuilder
	{
		...
		public MaterialRippleLayout create()
		{
			...
			layout.addView(child, new ViewGroup.LayoutParams(height, height));
