...
public class LiveWallpaperListAdapter extends BaseAdapter implements ListAdapter {
	...
	public LiveWallpaperListAdapter(Context context) {
		...
		new LiveWallpaperEnumerator(context).executeOnExecutor(MainActivity.fixedThreadPool,list);
	}
...
//		    if(position==0){
//		        holder.thumbnail.setImageResource(R.drawable.video_paper);
//		    }else{
		...
		protected Void doInBackground(List<ResolveInfo>... params) {
			...
			Collections.sort(list, new Comparator<ResolveInfo>() {
				...
				public int compare(ResolveInfo info1, ResolveInfo info2) {
					try {
						if (VIDEO_LIVE_WALLPAPER_PACKAGE.equals(new WallpaperInfo(mContext, info2).getPackageName())) {
							return 1;
						}
						if (VIDEO_LIVE_WALLPAPER_PACKAGE.equals(new WallpaperInfo(mContext, info1).getPackageName())) {
							return -1;
						}
